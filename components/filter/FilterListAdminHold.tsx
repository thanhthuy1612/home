...
import { updateSearchValue } from '@/lib/features/listRoom';
...
const FilterListAdminHold: React.FC<IFilter> = ({ fetchData }) => {
  const { width } = useAppSelector((state) => state.login);
  const { searchValue } = useAppSelector((state) => state.listRoom);
...
export default FilterListAdminHold;
